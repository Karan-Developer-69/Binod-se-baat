...
const Home = () => {
  const { isLoggedIn, theme } = useSelector((state: RootState) => state.user);
  ...
  return (
    <div className={`min-h-screen theme-transition font-grotesk overflow-x-hidden ${isDark ? 'bg-[#061E29] text-[#F3F4F4]' : 'bg-[#F3F4F4] text-[#061E29]'
      }`}>

      {/* --- FIXED NAVIGATION --- */}
      <nav className={`fixed w-full z-[100] theme-transition transition-all duration-500 ${scrolled ? 'py-4 premium-glass shadow-2xl' : 'py-8'
        }`}>
        ...
        {/* --- MOBILE DRAWER --- */}
        <div className={`fixed inset-0 ${isDark ? 'bg-[#061E29] text-[#F3F4F4]' : 'bg-[#F3F4F4] text-[#061E29]'} transition-all duration-500 md:hidden flex flex-col items-center justify-center space-y-8 z-[105] ${isMenuOpen ? 'opacity-100 visible translate-y-0 translate-x-0' : 'opacity-0 invisible pointer-events-none -translate-y-[100%] translate-x-[50%]'
          }`}>
          {['Engine', 'Security', 'About'].map((link) => (
          ...
          <button onClick={() => { toggleTheme(); toggleMenu(); }} className="text-[#5F9598] flex items-center gap-2 font-bold tracking-widest uppercase text-sm">
      ...
      <section id="Engine" className="py-24 px-6 bg-black/5">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-3 gap-8">
          {[
            ...
            <div key={i} className={`reveal p-12 rounded-[40px] border theme-transition group transition-all duration-700 ${isDark ? 'bg-[#1D546D]/10 border-white/5 hover:bg-[#1D546D]/20' : 'bg-white border-gray-100 shadow-xl'
      ...
      <section className="py-32 px-6">
        <div className={`max-w-6xl mx-auto reveal rounded-[50px] p-12 md:p-24 text-center border transition-all ${isDark ? 'bg-[#1D546D]/20 border-white/5' : 'bg-[#1D546D] text-white'
          }`}>
